...
$(document).ready(function () {
    ...
    if (!isEmpty(id)) {
        loaderShow(null);
        ax.setSuccess("exitoRiaProcedimiento");
        ax.setAccion("obtenerRiaProcedimientoXid");
        ax.addParam("id", id);
        ax.consumir();
    }
...
function exitoRiaProcedimiento(response) {
    if (response.status === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            case 'obtenerRiaProcedimientoXid':
                onResponseObtenerRiaProcedimientoXid(response.data);
                loaderClose();
                break;
            case 'crearRiaProcedimiento':
                exitoCrear(response.data);
                loaderClose();
                break;
            case 'actualizarRiaProcedimiento':
                exitoCrear(response.data);
                loaderClose();
                break;
        }
...
function onResponseObtenerRiaProcedimientoXid(data) {
...
function cargarPantallaListar() {
    var url = URL_BASE + "vistas/com/ria/riaProcedimiento.php";
...
function enviar(tipoAccion) {
    ...
    if (tipoAccion === 'Nuevo') {
        crearRiaProcedimiento(codigo, version, comentario, documento, docNombre, docId, estado);
    } else {
        var id = document.getElementById('id').value;
        actualizarRiaProcedimiento(id, codigo, version, comentario, documento, docNombre, docId, estado);
    }
}

function crearRiaProcedimiento(codigo, version, comentario, documento, docNombre, docId, estado) {
    if (validarFormulario(codigo, version, documento, docId)) {
        deshabilitarBotones();
        ax.setAccion("crearRiaProcedimiento");
...
function actualizarRiaProcedimiento(id, codigo, version, comentario, documento, docNombre, docId, estado) {
    if (validarFormulario(codigo, version, documento, docId)) {
        deshabilitarBotones();
        ax.setAccion("actualizarRiaProcedimiento");
