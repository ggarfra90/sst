...
$(document).ready(function () {
    ...
    if (!isEmpty(id)) {
        ...
        ax.setAccion("obtenerIperProcedimientoXid");
        ax.addParam("id", id);
        ax.consumir();
    }
...
function exitoIperProcedimiento(response) {
    if (response.status === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            case 'obtenerIperProcedimientoXid':
                onResponseObtenerIperProcedimientoXid(response.data);
                loaderClose();
                break;
            case 'crearIperProcedimiento':
                exitoCrear(response.data);
                loaderClose();
                break;
            case 'actualizarIperProcedimiento':
                exitoCrear(response.data);
                loaderClose();
                break;
        }
...
function onResponseObtenerIperProcedimientoXid(data) {
...
function cargarPantallaListar() {
    var url = URL_BASE + "vistas/com/iper/iperProcedimiento.php";
...
function enviar(tipoAccion) {
    ...
    if (tipoAccion === 'Nuevo') {
        crearIperProcedimiento(codigo, version, comentario, documento, docNombre, docId, estado);
    } else {
        var id = document.getElementById('id').value;
        actualizarIperProcedimiento(id, codigo, version, comentario, documento, docNombre, docId, estado);
    }
}

function crearIperProcedimiento(codigo, version, comentario, documento, docNombre, docId, estado) {
    if (validarFormulario(codigo, version, documento, docId)) {
        deshabilitarBotones();
        ax.setAccion("crearIperProcedimiento");
...
function actualizarIperProcedimiento(id, codigo, version, comentario, documento, docNombre, docId, estado) {
    if (validarFormulario(codigo, version, documento, docId)) {
        deshabilitarBotones();
        ax.setAccion("actualizarIperProcedimiento");
