...
$(document).ready(function () {
    ...
    if (!isEmpty(id)) {
        loaderShow(null);
        ax.setSuccess("exitoPoliticaSst");
        ax.setAccion("obtenerPoliticaSstXid");
        ax.addParam("id", id);
        ax.consumir();
    }
...
function exitoPoliticaSst(response) {
    if (response.status === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            case 'obtenerPoliticaSstXid':
                onResponseObtenerPoliticaSstXid(response.data);
                loaderClose();
                break;
            case 'crearPoliticaSst':
                exitoCrear(response.data);
                loaderClose();
                break;
            case 'actualizarPoliticaSst':
                exitoCrear(response.data);
                loaderClose();
                break;
        }
...
function onResponseObtenerPoliticaSstXid(data) {
...
function cargarPantallaListar() {
    var url = URL_BASE + "vistas/com/politica/politicaSst.php";
...
function enviar(tipoAccion) {
    ...
    if (tipoAccion === 'Nueva') {
        crearPoliticaSst(codigo, version, comentario, documento, docNombre, docId, estado);
    } else {
        var id = document.getElementById('id').value;
        actualizarPoliticaSst(id, codigo, version, comentario, documento, docNombre, docId, estado);
    }
}

function crearPoliticaSst(codigo, version, comentario, documento, docNombre, docId, estado) {
    if (validarFormulario(codigo, version, documento, docId)) {
        deshabilitarBotones();
        ax.setAccion("crearPoliticaSst");
...
function actualizarPoliticaSst(id, codigo, version, comentario, documento, docNombre, docId, estado) {
    if (validarFormulario(codigo, version, documento, docId)) {
        deshabilitarBotones();
        ax.setAccion("actualizarPoliticaSst");
