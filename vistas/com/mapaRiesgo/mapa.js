...
$(document).ready(function () {
    ...
    ax.setAccion("obtenerSucursalXempresa");
    ax.consumir();
    listarMapaRiesgo();
...
function listarMapaRiesgo() {
    ax.setAccion("listarMapaRiesgo");
    ax.consumir();
}

function buscarMapaRiesgo() {
    loaderShow(null);
    ax.setAccion("listarMapaRiesgo");
    var sucursalId = document.getElementById('cboSucursal').value;
...
function exitoMapaRiesgo(response) {
    if (response.status === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            case 'obtenerSucursalXempresa':
                onResponseObtenerSucursal(response.data);
                break;
            case 'listarMapaRiesgo':
                onResponseListarMapaRiesgo(response.data);
                onResponseVacio();
                break;
            case 'cambiarEstado':
                cambiarIconoEstado();
...
function onResponseObtenerSucursal(data) {
    if (!isEmpty(data)) {
        ...
        $.each(data, function (index, item) {
            $('#cboSucursal').append('<option value="' + item.id + '">' + item.descripcion + '</option>');
        });
    }
}
...
function nuevo() {
    var titulo = "Nuevo";
    var url = URL_BASE + "vistas/com/mapaRiesgo/mapa_form.php?winTitulo=" + titulo;
    cargarDiv("#window", url);
}

function onResponseListarMapaRiesgo(data) {
    $("#dataList").empty();
    var cuerpo_total = "";
    var cuerpo = "";
    var cuerpo_acc = "";
    ...
    if (!isEmpty(data)) {
        $.each(data, function (index, item) {
            ...
            cuerpo = "<tr>"
                    ...
                    + "<td style='text-align:center;'>" + item.suc_descripcion + "</td>"
...
function verDetalle(id) {
    ...
    var url = URL_BASE + "vistas/com/mapaRiesgo/mapa_form.php?winTitulo=" + titulo + "&id=" + id + "&readonly=true";
    cargarDiv("#window", url);
}

function editar(id) {
    var titulo = "Editar";
    var url = URL_BASE + "vistas/com/mapaRiesgo/mapa_form.php?winTitulo=" + titulo + "&id=" + id + "&readonly=false";
    cargarDiv("#window", url);
}
